...
MAX_SAFE_BOOST = 8.0     # HARD LIMIT: Never boost more than 8dB regardless of user input
VERSION = "v1.0.1 (Safety Limit)"
...
def parse_measurements(filename):
    try:
        ...
        return data[:, 0], data[:, 1], data[:, 2] # freqs, mags, phases
    ...
    for i, f in enumerate(freq_axis):
        if f > 0:
            ...
            if enable_mag_correction and use_house_curve and (mag_c_min <= f <= mag_c_max):
                # Clamp boost to MAX_SAFE_BOOST even if user provided higher
...
# --- OPTIMIZED PLOTTING (FFT BASED) ---
def generate_prediction_plot(orig_freqs, orig_mags, orig_phases, filt_ir, fs, title):
    """Calculates predicted response using FFT for speed."""
    try:
        # 1. Calculate Filter Response using FFT (O(N log N) - Very Fast)
        n_fft = len(filt_ir)
        freq_axis_lin = scipy.fft.rfftfreq(n_fft, d=1/fs)
        
        h_filt_complex = scipy.fft.rfft(filt_ir)
        
        # 2. Interpolate Original Data to the Linear FFT Grid
        ...
        # 3. Multiply to get Total System Response
        total_complex = orig_complex_lin * h_filt_complex
        
        # 4. "Remove IR Delay" Logic (Time Domain Shift)
        ...
        # 6. Interpolate back to original log-spaced frequencies for plotting
...
def main():
    ...
    if not os.path.exists(INPUT_FILE_L) or not os.path.exists(INPUT_FILE_R):
        put_error("CRITICAL ERROR: Measurement files not found!")
        put_text(f"Please place '{INPUT_FILE_L}' and '{INPUT_FILE_R}' in this folder.")
        return

    hc_files = [f for f in os.listdir('.') if f.endswith('.txt') and f not in [INPUT_FILE_L, INPUT_FILE_R, 'requirements.txt']]
    hc_options = ['None', "Built-in ('not Dr Toole')"] + hc_files

    inputs_list = [
        select("Output Format", options=['WAV', 'CSV'], name='fmt'),
        radio("Channel Layout", options=['Mono (Separate files)', 'Stereo (Single file)'], name='layout'),
        select("Sample Rate", options=[44100, 48000, 96000], value=48000, name='fs', type=NUMBER),
        select("Taps", options=[2048, 4096, 8192, 16384, 32768, 65536, 131072], value=65536, name='taps', type=NUMBER),
        input("Global Gain (dB)", value=0.0, type=FLOAT, name='gain', help_text="Adjusts volume. Use negative values (e.g. -3dB) if boosting."),
        select("House Curve", options=hc_options, name='hc_file'),
        checkbox("Correction Mode", options=['Enable Frequency Response Correction'], value=['Enable Frequency Response Correction'], name='mag_correct'),
        input("Correction Min Freq (Hz)", value=10, type=FLOAT, name='hc_min'),
        input("Correction Max Freq (Hz)", value=200, type=FLOAT, name='hc_max'),
        input("Max Boost (dB)", value=5.0, type=FLOAT, name='max_boost', help_text=f"Note: Boost is safely limited to max {MAX_SAFE_BOOST} dB internally."),
        checkbox("High Pass Filter", options=['Enable'], name='hpf_enable'),
        input("HPF Freq (Hz)", value=None, type=FLOAT, name='hpf_freq', placeholder="e.g. 20"), 
        select("HPF Slope (dB/oct)", options=[6, 12, 18, 24, 36, 48], value=24, type=NUMBER, name='hpf_slope')
    ]
    ...
    for i in range(1, 6):
        ...
        inputs_list.append(input(label_txt, name=f'xo{i}_f', type=FLOAT, placeholder="Leave empty if unused"))
        inputs_list.append(select(f"XO {i} Slope (dB/oct)", options=slope_opts, value=12, name=f'xo{i}_s'))

    data = input_group("Filter Settings", inputs_list)

    put_processbar('bar')
    set_processbar('bar', 0.2)
    put_text("Reading measurements...")
    ...
    if data['hc_file'] == "Built-in ('not Dr Toole')":
        hc_freqs, hc_mags = get_default_house_curve()
    elif data['hc_file'] != 'None':
        hc_freqs, hc_mags = parse_house_curve(data['hc_file'])
    ...
    if freqs_l is None:
        put_error("Failed to read L.txt/R.txt")
        return

    set_processbar('bar', 0.5)
    put_text("Analyzing and Generating Filters...")
    ...
    put_markdown("## 📊 Analysis Plots (Predicted)")
    ...
    put_markdown("### Done! You can close this window.")
