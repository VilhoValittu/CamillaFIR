...
def generate_filter(freqs, raw_mags, raw_phases, crossovers, 
    ...
    # -------------------------------------------------------------
    # NEW LOGIC: MINIMUM PHASE "SMART WINDOWING"
    # -------------------------------------------------------------
    meas_mags_for_eq = meas_mags # Default: use raw smoothed magnitudes
    
    if is_min_phase:
        # 1. Clean the IR using the window setting
        clean_freqs, clean_mags_raw, clean_phases_raw = clean_measurement_ir(freqs, raw_mags, raw_phases, window_ms=ir_window_ms, fs=fs)
        
        # 2. Get smoothed magnitude of Cleaned IR
        if smoothing_type == t('smooth_psy'):
            clean_mags_smoothed = psychoacoustic_smoothing(clean_freqs, clean_mags_raw)
            meas_mags_clean = interpolate_response(clean_freqs, clean_mags_smoothed, freq_axis)
        else:
            s_mags, _ = apply_smoothing_std(clean_freqs, clean_mags_raw, clean_phases_raw, 1/48.0)
            meas_mags_clean = interpolate_response(clean_freqs, s_mags, freq_axis)
            
        # 3. NORMALIZE Cleaned Level to Raw Level (Hybrid Fix)
        # Calculate offset between Raw (meas_mags) and Cleaned (meas_mags_clean)
        a_start = max(l_match_min, 10.0)
        a_end = l_match_max
        mask_norm = (freq_axis >= a_start) & (freq_axis <= a_end)
        
        if np.any(mask_norm):
            # Calculate energy average for robust level comparison
            p_raw = np.mean(10**(meas_mags[mask_norm]/10.0))
            p_clean = np.mean(10**(meas_mags_clean[mask_norm]/10.0))
            
            lev_raw = 10*np.log10(p_raw + 1e-12)
            lev_clean = 10*np.log10(p_clean + 1e-12)
            
            norm_offset = lev_raw - lev_clean
            meas_mags_clean += norm_offset # Lift the cleaned curve up to match raw volume
            
        meas_mags_for_eq = meas_mags_clean

    # -------------------------------------------------------------
    
    raw_gain_db = target_mags - meas_mags_for_eq
    ...
    # --- PHASE CORRECTION CALCULATIONS (Common for Linear & Mixed) ---
    # We need these for Linear Phase AND the Low-Frequency part of Mixed Phase
    
    # 1. Calculate Minimum Phase of the MEASUREMENT (to find excess phase)
    ...
    if 'Min' in filter_type_str:
        # REW-style Minimum Phase Transform of the final response
        filt_min_phase_rad = calculate_minimum_phase(total_mag_response)
...
def generate_prediction_plot(orig_freqs, orig_mags, orig_phases, filt_ir, fs, title, save_filename=None, target_stats=None):
    try:
        ...
        # ADDED: Level Match Area Visualization (Plotly)
        if target_stats and 'l_match_min' in target_stats:
            fig.add_vrect(
                ...
                row=1, col=1, annotation_text="Level Area", annotation_position="top left"
            )
