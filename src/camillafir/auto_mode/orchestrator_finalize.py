...
def _build_phase2_pareto_status(
    *,
    ...
    return (
        "CamillaFIR automatic mode: phase 2 pareto selected winner "
        f"(rank_best {_fmt_status_metric(_m(rank_best, 'rank_score', float('nan')), decimals=3)} -> "
        f"pareto {_fmt_status_metric(_m(winner, 'rank_score', float('nan')), decimals=3)}, "
    ...
    if phase2_pool_raw:
        ...
        if len(phase2_kept) >= pareto_min_n:
            ...
            if isinstance(pareto_winner, dict):
                ...
                prev_preset = dict(search_state.best_preset or {})
                ...
                if callable(status_cb):
                    prev_rank = _m(prev_best, "rank_score", float("nan"))
                    next_rank = _m(w_metrics, "rank_score", float("nan"))
                    prev_avg = _m(prev_best, "avg_score", float("nan"))
                    next_avg = _m(w_metrics, "avg_score", float("nan"))
                    winner_changed = bool(prev_preset != w_preset)
                    if (
                        np.isfinite(prev_rank)
                        and np.isfinite(next_rank)
                        and abs(float(prev_rank) - float(next_rank)) > 1e-9
                    ):
                        winner_changed = True
                    if (
                        np.isfinite(prev_avg)
                        and np.isfinite(next_avg)
                        and abs(float(prev_avg) - float(next_avg)) > 1e-9
                    ):
                        winner_changed = True
                    if winner_changed:
                        status_cb(
