...
MAX_SAFE_BOOST = 8.0
VERSION = "v2.1.3" # Zoom Hint & Mixed Guide"
...
TRANSLATIONS = {
    'en': {
        ...
        'fdw': "FDW Cycles",
        'fdw_help': "Linear: 15. Min Phase: N/A",
        ...
        'ir_window': "IR Window (ms)",
        'ir_window_help': "For Minimum Phase only. Defaults to 250ms.",
        'phase_limit': "Phase Corr Limit (Hz)",
        'mixed_freq': "Mixed Phase Split (Hz)",
        'zoom_hint': "(Double-click graph to reset zoom)",
        ...
        'guide_mix_title': "❓ Guide: Mixed Phase Split",
        ...
        'guide_fdw_auto': "NOTE: In Minimum Phase mode, it's recommend to use fwd value 15. Try it and find what suits you.",
        ...
        # MIXED PHASE GUIDE
        'guide_mix_intro': "Determines where the filter switches from Time Correction (Linear) to Natural Phase (Minimum).",
        'guide_mix_low': "• **100-200 Hz:** Corrects only sub-bass timing. Safest for soundstage/imaging.",
        'guide_mix_mid': "• **300 Hz (Default):** Standard transition point. Corrects main room modes.",
        'guide_mix_high': "• **500+ Hz:** Tightens mid-bass punch but risks affecting vocal phase coherence.",
    ...
    'fi': {
        ...
        'ir_window': "IR Ikkunointi (ms)",
        'ir_window_help': "Vaikuttaa vain Minimum Phase -tilaan. Oletus 250ms.",
        'phase_limit': "Vaihekorjauksen yläraja (Hz)",
        'mixed_freq': "Mixed Phase Jako (Hz)",
        'zoom_hint': "(Tuplaklikkaa kuvaajaa nollataksesi zoomin)",
        ...
        'guide_mix_title': "❓ Opas: Mixed Phase Jako",
        ...
        # Mixed Phase Guide
        'guide_mix_intro': "Määrittää taajuuden, jossa filtteri vaihtaa aikakorjauksesta (Linear) luonnolliseen vaiheeseen (Minimum).",
        'guide_mix_low': "• **100-200 Hz:** Korjaa vain sub-bassojen ajoituksen. Turvallisin stereokuvalle.",
        'guide_mix_mid': "• **300 Hz (Oletus):** Vakio. Korjaa pääasialliset huonemoodit.",
        'guide_mix_high': "• **500+ Hz:** Napakoittaa iskuääniä (mid-bass), mutta voi vaikuttaa laulun vaihekoherenssiin.",
...
def put_guide():
    ...
    # NEW GUIDE
    content_mix = [
        put_text(t('guide_mix_intro')),
        put_markdown(t('guide_mix_low')),
        put_markdown(t('guide_mix_mid')),
        put_markdown(t('guide_mix_high'))
    ]
    put_collapse(t('guide_mix_title'), content_mix)
...
def clean_measurement_ir(freqs, mags, phases, window_ms=500.0, fs=48000):
    ...
    # --- FIX v2.0.8: CENTER THE IMPULSE BEFORE WINDOWING ---
...
# NEW: Helper for Group Delay Calculation
def calculate_group_delay(freqs, phases_deg):
    # Unwrap and convert to radians
    phase_rad = np.unwrap(np.deg2rad(phases_deg))
    # Calculate derivative d(phi)/d(f)
    # Note: np.gradient handles non-uniform spacing if freqs is provided
    d_phi_d_f = np.gradient(phase_rad, freqs)
    # Group Delay formula: - (1 / 2pi) * (d_phi / d_f)
    gd_sec = -d_phi_d_f / (2 * np.pi)
    gd_ms = gd_sec * 1000.0
    # Apply smoothing because raw gradient is noisy
...
def generate_filter(freqs, raw_mags, raw_phases, crossovers, 
                    ...
                    mixed_split_freq=300.0): # NEW ARG
    ...
    if 'Min' in filter_type_str:
        ...
        impulse_out = combine_mixed_phase(ir_lin, ir_min, fs, split_freq=mixed_split_freq) # Use var
...
def generate_filter_plot_plotly(filt_ir, fs, title):
    try:
        ...
        fig.update_layout(height=600, showlegend=True, template="plotly_white", width=1100, title_text=f"{title} {t('zoom_hint')}")
        return fig.to_html(include_plotlyjs=True, full_html=True) 
    except Exception as e:
        print(f"Filter Plot Error: {e}")
        return None

def generate_prediction_plot(orig_freqs, orig_mags, orig_phases, filt_ir, fs, title, save_filename=None, target_stats=None, mixed_split=None):
    try:
        n_fft = len(filt_ir)
        ...
        gd_orig = calculate_group_delay(orig_freqs, plot_phase_orig) # Fixed: use smoothed phase
        gd_pred = calculate_group_delay(orig_freqs, plot_phase_pred) # Fixed: use smoothed phase
        ...
        if mixed_split:
             for r in range(1, 5):
                 ...
                 if r == 1:
                     fig.add_annotation(x=np.log10(mixed_split), y=80, text="Split", showarrow=False, row=r, col=1)
        ...
        fig.update_yaxes(title_text="ms", row=3, col=1, range=[-20, 100]) # NEW WIDER RANGE
        fig.update_yaxes(title_text="dB", row=4, col=1)
        
        fig.update_layout(height=1100, showlegend=True, template="plotly_white", width=1100, title_text=f"{title} {t('zoom_hint')}")
...
def generate_combined_plot_mpl(orig_freqs, orig_mags, orig_phases, filt_ir, fs, title, target_stats=None):
    try:
        ...
        gd_orig = calculate_group_delay(orig_freqs, plot_phase_orig)
        gd_pred = calculate_group_delay(orig_freqs, plot_phase_pred)
        ...
        # 3. Create Matplotlib Figure (4 subplots) - INCREASED SIZE
        ...
        ax3.set_ylim(-20, 100) # Same zoom as HTML
...
def main():
    ...
    inputs_list = [
        ...
        input(t('mixed_freq'), value=defaults.get('mixed_freq', 300.0), type=FLOAT, name='mixed_freq'),
        ...
        input(t('ir_window'), value=defaults.get('ir_window', 250.0), type=FLOAT, name='ir_window', help_text=t('ir_window_help')),
        
        input(t('reg_strength'), value=defaults.get('reg_strength', 30.0), type=FLOAT, name='reg_strength'),
        
        input(t('phase_limit'), value=defaults.get('phase_limit', 20000.0), type=FLOAT, name='phase_limit'),
    ...
    try:
        ...
        # Get parameters
        clean_win_ms = float(data.get('ir_window', 250.0))
        ...
        phase_limit = float(data.get('phase_limit', 20000.0))
        mixed_freq = float(data.get('mixed_freq', 300.0))
        ...
        is_mixed_phase = ('Mixed' in filter_type_str)
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for i, fs_val in enumerate(target_rates):
                ...
                l_imp, l_min, l_max, l_stats = generate_filter(
                    ...
                    ir_window_ms=clean_win_ms, mixed_split_freq=mixed_freq
                )
                ...
                if fs_val == base_fs or (len(target_rates) > 1 and i == 0):
                    ...
                    split_viz = mixed_freq if is_mixed_phase else None
                    
                    plot_l_html = generate_prediction_plot(freqs_l, mags_l, phases_l, l_imp, fs_val, t('tab_l'), target_stats=l_stats, mixed_split=split_viz)
                    plot_r_html = generate_prediction_plot(freqs_r, mags_r, phases_r, r_imp, fs_val, t('tab_r'), target_stats=r_stats, mixed_split=split_viz)
