...
TRANSLATIONS = {
    'en': {
        ...
        'guide_fdw_mid': "• 15: Standard (Balanced).",
        ...
        'guide_lvl_algo': "• Algorithm: 'Median' is recommended as it ignores extreme peaks/nulls. 'Average' includes them.",
        'guide_lvl_std': "• Range: Defines the frequency window used for calculation (shown as gray area in plot).",
        ...
        'fdw_help': "15=Default.",
        'hc_mode': "House Curve",
        'hc_mode_builtin': "Built-in",
        'hc_mode_upload': "Upload...",
        ...
        'lvl_algo': "Calculation Algo", # NEW
        'algo_mean': "Average",
        'algo_median': "Median (Robust)",
        'lvl_min': "Range Min (Hz)",
        'lvl_max': "Range Max (Hz)",
        'lvl_help': "Freq range for calculation (Gray area in plot).",
        ...
        # DEBUG REPORT
        'dbg_header': "--- CALCULATION REPORT ---",
        'dbg_corr_status': "Correction Enabled:",
        'dbg_corr_range': "Correction Range:",
        'dbg_lvl_mode': "Level Mode:",
        'dbg_lvl_range': "Auto-Calc Range:",
        'dbg_offset': "Applied Offset:",
        'dbg_yes': "YES",
        'dbg_no': "NO (EQ Disabled!)"
    },
    'fi': {
        'title': "CamillaFIR",
        ...
        'guide_fdw_mid': "• 15: Vakio (Tasapainoinen).",
        ...
        'hc_mode_builtin': "Sisäänrakennettu",
        ...
        # DEBUG REPORT
        'dbg_header': "--- LASKENTARAPORTTI ---",
        'dbg_corr_status': "Korjaus päällä:",
        'dbg_corr_range': "Korjausalue:",
        'dbg_lvl_mode': "Tason laskenta:",
        'dbg_lvl_range': "Laskenta-alue (Auto):",
        'dbg_offset': "Tason muutos (Offset):",
        'dbg_yes': "KYLLÄ",
        'dbg_no': "EI (EQ Pois päältä!)"
    }
}
...
def load_config():
    default_conf = {
        ...
        # v1.11.0 DEFAULTS
        'lvl_mode': t('lvl_auto'),
        'lvl_algo': t('algo_median'), # Default to Median now
        'lvl_manual_db': 75.0,
...
def generate_filter(freqs, raw_mags, raw_phases, crossovers, 
    ...
    # --- LEVEL MATCH LOGIC v1.11.0 (Robust) ---
    ...
    if is_min_phase:
        ...
        return impulse_final, 0.0, 0.0, stats
    else:
        ...
        return np.roll(impulse, n_fft // 2) * window, gd_min, gd_max, stats
...
def generate_prediction_plot(orig_freqs, orig_mags, orig_phases, filt_ir, fs, title, save_filename=None, target_stats=None):
    try:
        ...
        # --- DRAW GRAY AREA FOR LEVEL MATCH RANGE v1.11.0 ---
...
def main():
    ...
    if not val_corr: val_corr = [t('enable_corr')] 
    ...
    try:
        ...
        # Pass 1: Get filter AND debug stats
        ...
        try:
            ...
            # --- DEBUG REPORT UI ---
            put_markdown(f"### {t('dbg_header')}")
            
            status_txt = t('dbg_yes') if l_stats['correction_enabled'] else t('dbg_no')
            style = 'color: green; font-weight: bold;' if l_stats['correction_enabled'] else 'color: red; font-weight: bold;'
            put_row([put_text(t('dbg_corr_status')), put_text(status_txt).style(style)])
            
            put_row([put_text(t('dbg_corr_range')), put_text(f"{data['hc_min']} Hz - {data['hc_max']} Hz")])
            
            # Show Auto range only if Auto
            if lvl_mode == t('lvl_auto'):
                put_row([put_text(t('dbg_lvl_range')), put_text(f"{match_min} Hz - {match_max} Hz (Auto)")])
            else:
                put_row([put_text(t('dbg_lvl_mode')), put_text(f"Manual ({match_man_db} dB)")])
            
            put_text(f"{t('dbg_offset')} L={l_stats['offset_db']:.2f} dB, R={r_stats['offset_db']:.2f} dB")
            # -----------------------
