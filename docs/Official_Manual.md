# CamillaFIR – Official Manual (v4.2.0)

## 1. Overview
CamillaFIR generates **FIR room-correction filters** from REW exports, built-in sweep measurements, and WAV/IR captures.
...
CamillaFIR supports two common REW export workflows and an optional internal measurement path.
...
CamillaFIR can import impulse-response WAV files and convert them internally for processing.
...
CamillaFIR includes an integrated measurement tool that plays a sine sweep, records the response, and produces IR WAV files ready to import directly into the filter-generation workflow. It can replace a separate REW session when the hardware setup allows it.
